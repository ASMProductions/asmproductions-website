...
export default async function handler(req, res) {
  ...
  try {
    ...
    // PUT: Submit/confirm the week (marks it submitted, stamps history entry)
    if (req.method === 'PUT') {
      ...
      const chores = (await redis.get(choreKey)) || initializeWeeklyChores();
      ...
      if (weekIndex >= 0) {
        history[weekIndex].total = weeklyTotal;
        history[weekIndex].submitted = true;
        history[weekIndex].submittedAt = submittedAt;
      } else {
        history.push({ week: weekKey, total: weeklyTotal, submitted: true, submittedAt });
      }
